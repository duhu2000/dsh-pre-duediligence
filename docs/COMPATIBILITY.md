...
## 0.1.24：Host 首页布局与历史详情（2026-09-13）

| 实测问题 | 修复契约 | 验证边界 |
| --- | --- | --- |
| 菜单点击后显示“正在打开…” | 可见菜单名固定为“访前尽调”，加载状态只进入无障碍与控件状态 | 重复点击仍由 busy guard 阻止，不创建重复 Session |
| 首页退回“探索未至之境 预览版” | 兼容 marker 与 Hero 为兄弟分支的 Host DOM，监听同一页面范围内的宿主重渲染 | 只改写本插件专属空白会话，卸载时恢复原文案与标记 |
| 历史清单不能进入任务与下载报告 | 以历史记录自身的来源 Session 读取详情和报告；返回清单不改变当前任务 | 来源缺失的旧记录仍只能看摘要，不猜测 Session |

本地 Node 24.19.0 门禁覆盖严格类型检查、全量自动化测试、Host/Client 构建，以及浅色/深色 × 桌面/窄屏 4 个隔离 Chrome 场景。真实 DSH 与生产企查查 Provider 仍以安装后的用户验收为准。

## 0.1.23：旧记录来源与显式 TaskScope（2026-09-13）

| 实测问题 | 修复契约 | 验证边界 |
| --- | --- | --- |
| 旧 Host 记录缺少 Workspace / Session 时被丢弃 | 兼容读取并以空来源哨兵持久化，继续进入 Profile 历史 | 旧记录不进入任何当前 Session 列表，也不被猜测或重绑 |
| 历史卡片来源空白，无法判断归属 | 缺失项明确显示“来源：Workspace 未记录（旧记录） · Session 未记录（旧记录）”，使用中性样式 | 无法恢复从未持久化的真实来源，完整会话仍需回到原生会话查阅 |
| 可选 `sessionId` 参数让当前任务与历史查询边界含糊 | 客户端改用 `HostedTaskListScope` 判别联合；Session A/B/legacy fixtures 固化可见性 | 单任务读取、报告回写和下载继续由 Host 强制校验当前 `sessionId` |

本地 Node 24.19.0 发布门禁通过严格类型检查、23 个测试文件 / 165 项测试、Host/Client 构建，以及浅色/深色 × 桌面/窄屏 4 个隔离 Chrome 场景。自动化覆盖当前 Session A、跨 Session B 与缺少来源的 legacy 记录，并检查长来源文本和旧记录标识不造成横向溢出。生产 DSH Profile 与真实企查查 Provider 未在本轮自动化中调用，仍以安装后的用户验收为准。

## 0.1.22：跨 Session 历史汇总（2026-09-13）

| 实测问题 | 修复契约 | 验证边界 |
| --- | --- | --- |
| 重新进入访前尽调后历史为空 | 历史视图不再附带当前 `sessionId` 过滤，读取当前 DSH Profile 的全部 Host 访前任务 | 只汇总本 Profile 的任务元数据和制品，不跨 Profile，也不复制原生会话正文 |
| 历史汇总可能污染当前任务 | 当前任务发现、进度、报告回写和下载仍严格携带当前 `sessionId` | 历史卡片仅供查看并标注来源 Workspace / Session，不会重新认领旧任务 |

本地 Node 24.18.0 发布门禁通过严格类型检查、23 个测试文件 / 163 项测试、Host/Client 构建、浅色/深色 × 桌面/窄屏 4 个隔离 Chrome 场景及 npm `0.1.22` 包清单 dry-run。现有 Host 持久记录会恢复展示，但本版不会凭空恢复从未写入 Host 的旧版浏览器瞬时状态。
...
- 当前任务和进度仍按 Session 隔离；任务历史读取 Host 持久库并跨访前尽调 Session 汇总。清空和“新的尽调”只重置本地当前视图，不删除 Host 历史或制品。
