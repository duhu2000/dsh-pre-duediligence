...
describe("访前工作台企查查蓝主题", () => {
  ...
  it("阶段、子步骤与维度标签共享完成、待核验和失败颜色", () => {
    ...
    expect(workbenchSource).toContain('title="执行覆盖"')
  ...
  it("区分流程待复核与风险预警，并解释互斥经营状态", () => {
    ...
    expect(workbenchSource).toContain("客观数据摘要")
  ...
  it("任务历史跨访前 Session 汇总，但当前任务仍按 Session 隔离", () => {
    expect(workbenchSource).toContain('const records = await fetchHostedTasks({ kind: "current", sessionId })')
    expect(workbenchSource).toContain('const records = await fetchHostedTasks({ kind: "profile-history" })')
    expect(workbenchSource).toContain("汇总当前 DSH Profile 中所有访前尽调 Session")
    expect(workbenchSource).toContain("const origin = hostedTaskOrigin(item)")
