...
for (const [theme, width, height] of scenarios) {
  ...
  assert.equal(attr("capability-labels"), "对象与目标|范围确认|资料采集|证据核验|任务历史")
  ...
  assert.equal(attr("business-container-control-count"), "0")
  assert.equal(attr("business-return-control-count"), "0")
  assert.equal(attr("company-label"), "拜访客户")
  assert.equal(attr("default-selections"), "银行/信贷客户经理|风险与涉诉|股权与实控人|经营与财务|联系人与触达|知识产权|招投标业绩|3分钟速览|一页纸简报")
  assert.equal(attr("company-enter-isolated"), "true")
  assert.equal(attr("company-typing-local"), "true")
  ...
  assert.equal(attr("collection-only"), "true")
  assert.equal(attr("verification-only"), "true")
  ...
  assert.equal(attr("home-subtitle-count"), "0")
  assert.equal(attr("home-order"), "true")
