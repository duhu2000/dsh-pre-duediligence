...
兼容规则：`key_personnel` 会由宿主规范化为标准维度 `personnel`，不应作为业务错误或额外调用计费。
...
- 速览：基础主体 + 最相关机会信号 + 风险扫描 + 全部非零风险下钻 + 关键人员核验
- 标准：基础集 + 状态判定 + 2–4 个假设的反证 + 风险明细
- 深度：增加年度序列、变化、行业包、知识产权、招投标、关键人员和更多反证

三档均不设置插件调用次数上限；档位控制查询优先级、覆盖深度与目标时长。Provider 账户额度、权限和限流不属于插件上限，按实际返回披露。
...
6. Provider 限流或账户额度不足：保留已取得事实，明确未覆盖项并以 `partial` 交付，不把它误写成插件次数上限。
