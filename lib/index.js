// src/saved-report-source.ts
function createSavedReportSource(store) {
  const source = {
    async read(taskId, version, scope) {
    ...
    async versions(taskId, version, scope) {
      ...
      for (const record of await store.listSnapshots()) {
        if (record.sessionId !== scope.sessionId || record.workspace !== scope.workspace || (record.rootTaskId ?? record.id) !== root || record.entity?.creditCode !== anchor.entity?.creditCode || record.entity?.fullName !== anchor.entity?.fullName || !["completed", "partial"].includes(record.state) || !record.reportMarkdown?.trim() || !record.completedAt) continue;
...
var isInitialDraft = (text2) => text2 === INITIAL_DRAFT_TEXT;
var hasUnfilledPlaceholder = (text2) => /【[^】]*】/u.test(text2);
...
function extractCompanyNames(text2, max = IMAGE_LIMITS.maxEntries) {
  if (isInitialDraft(text2)) return [];
  const out = [];
...
function unwrap(result) {
  ...
  if (Array.isArray(content)) {
    const text2 = content.filter((b) => b?.type === "text" && typeof b.text === "string").map((b) => b.text).join("\n");
    if (text2 !== "") {
      try {
        value = JSON.parse(text2);
      } catch {
        value = { result_md: text2 };
      }
    }
  }
  return value;
...
var ImageIntakeStore = class {
  ...
  async run(commandId, exec) {
    ...
    record.promise = (async () => {
      ...
      const text2 = markdownOf(value);
      if (text2 === "") throw new ImageIntakeError("PV_IMAGE_NO_TEXT", "\u56FE\u7247\u4E2D\u672A\u8BC6\u522B\u5230\u53EF\u7528\u6587\u5B57\uFF0C\u8BF7\u6362\u66F4\u6E05\u6670\u7684\u539F\u56FE\u3002", 422);
      const entries = extractCompanyNames(text2);
      return { entries, text: text2.slice(0, 4e3) };
    })().then(async (result) => {
...
async function renderReportFile(task, format, fontPath = process.env.DSH_PREVISIT_PDF_FONT) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      pdf.fontSize(11).text(reportText, { lineGap: 4 });
      pdf.end();
    } catch (error) {
      pdf.destroy();
      reject(error);
...
function stripFactIds(text2) {
  return text2.replace(/\s*\[F-?\d{1,4}(?:\s*[\/、,，;；|]\s*F-?\d{1,4})*\]/g, "").replace(/[ \t]+([，。；：）])/g, "$1");
}
var PRIORITY_WORDS = { P0: "\u4F18\u5148", P1: "\u91CD\u8981", P2: "\u5907\u9009" };
function humanizeHypothesis(text2) {
  return text2.replace(/^\s*H(\d+)\s*[·・:：\-–]\s*(P[012])\s*[·・:：\-–]\s*/, (_m, n, p) => `\u5047\u8BBE ${n}\uFF08${PRIORITY_WORDS[p] ?? p}\uFF09`).replace(/(?<![A-Za-z0-9])H(\d{1,2})(?![A-Za-z0-9])/g, "\u5047\u8BBE $1");
}
function inline(text2) {
  let html = esc(humanizeHypothesis(stripFactIds(text2)));
  html = html.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>");
  return html;
}
var CIRCLED = { "\u2460": "1", "\u2461": "2", "\u2462": "3", "\u2463": "4", "\u2464": "5", "\u2465": "6", "\u2466": "7", "\u2467": "8", "\u2468": "9", "\u2469": "10" };
function normalizeHeading(text2) {
  return text2.trim().replace(/^([①②③④⑤⑥⑦⑧⑨⑩])\s*/, (_m, c) => `${CIRCLED[c] ?? c}\u3001`).replace(/^(\d+)[.．]\s*/, "$1\u3001");
}
...
function parseAnalysisRecord(input) {
  ...
  const record = { id: text2("id", 80), kind, status, title: text2("title", 160), summary: text2("summary"), support: list("support"), counter: list("counter"), unknown: list("unknown"), nextAction: text2("nextAction"), evidenceIds: list("evidenceIds") };
...
function createMaterial(input, taskId) {
  ...
  const text2 = boundedText(input.text, 1e5);
...
var PrevisitWorkflowStore = class {
  ...
  async listSnapshots() {
    await this.attachPromise;
    const saved = new Map(this.table?.entries() ?? []);
    for (const [id, value] of this.records) saved.set(id, value);
    return structuredClone([...saved.values()].filter((value) => value !== null && typeof value === "object"));
  }
...
async function readJson(req) {
  ...
  const value = text2 === "" ? {} : JSON.parse(text2);
      ...
      try {
        ...
        if (rest.length === 2 && rest[1] === "report") {
          ...
          const html = buildPrevisitReportHtml(task.reportMarkdown, {
            ...
            ...task.completedAt === void 0 ? {} : { generatedAt: task.completedAt }
          });
...
// src/company-portrait.ts
var text = (value) => typeof value === "string" ? value.trim().slice(0, 1200) : "";
function hierarchy(value, keys) {
  if (typeof value === "string") return text(value);
  if (!value || typeof value !== "object" || Array.isArray(value)) return "";
  const row = value;
  return keys.map((key) => text(row[key]) ? `${key}\uFF1A${text(row[key])}` : "").filter(Boolean).join(" / ");
}
function collectPortrait(target, row) {
  const put = (key, value) => {
    if (value && !target[key]) Object.assign(target, { [key]: value });
  };
  const area = row.\u5730\u533A\u4FE1\u606F;
  ...
  if (target.products === void 0 && Object.hasOwn(row, "\u4E3B\u8425\u4EA7\u54C1")) {
    if (Array.isArray(row.\u4E3B\u8425\u4EA7\u54C1)) target.products = [...new Set(row.\u4E3B\u8425\u4EA7\u54C1.filter((item) => typeof item === "string").map((item) => text(item).slice(0, 120)).filter(Boolean))].slice(0, 10);
...
function summarizeResult(value, dimension) {
  const result = { summary: "", facts: [], factors: [] };
  const seen = /* @__PURE__ */ new Set();
  const portrait = dimension === "registration" || dimension === "profile" ? {} : void 0;
  ...
  const visit = (item, depth, path = "") => {
    ...
    const row = item;
    if (portrait && !path.replace(/content\[\d+\]/g, "content").includes("[")) collectPortrait(portrait, row);
  ...
  visit(value, 0);
  if (portrait && Object.keys(portrait).length) result.portrait = portrait;
  return result;
}
...
function registerPrevisitTools(ctx, workflow = new PrevisitWorkflowStore(), options = {}) {
  ...
  disposers.push(ctx.tools.guard((exec) => {
    ...
    return !disposed && permit?.owner === ownerOf(exec.agent) && permit.name === exec.name && permit.parent === exec.parent ? void 0 : "\u8BBF\u524D\u4F01\u67E5\u67E5\u8C03\u7528\u5FC5\u987B\u901A\u8FC7 previsit_begin / previsit_confirm_entity / previsit_query\uFF1B\u7981\u6B62\u7ED5\u8FC7\u4E3B\u4F53\u7ED1\u5B9A\u4E0E\u56FA\u5B9A\u4E1A\u52A1\u8DEF\u7531\u3002\u8BF7\u4F7F\u7528 qcc-previsit-onepager Skill\uFF0C\u4E0D\u56E0\u6295\u8D44\u673A\u6784\u89D2\u8272\u5207\u6362\u4E3A ic-memo-qcc\u3002\u5C1A\u65E0\u4EFB\u52A1\u65F6\u5148 previsit_begin\uFF0C\u518D previsit_query(dimension=entity_search)\uFF0C\u53D6\u5F97\u5019\u9009\u540E previsit_confirm_entity\uFF1B\u5DF2\u6709\u4EFB\u52A1\u8BF7\u6CBF\u7528 taskId\uFF0C\u4E0D\u91CD\u590D\u521B\u5EFA\u3002";
  }));
  ...
  register("previsit_query", "\u67E5\u8BE2\u5DF2\u786E\u8BA4\u4EFB\u52A1\u7684\u4E00\u9879\u4E1A\u52A1\u7EF4\u5EA6\u3002\u9996\u5148 entity_search\uFF1B\u786E\u8BA4\u552F\u4E00\u4E3B\u4F53\u540E\u518D\u67E5\u8BE2\u5176\u4F59\u7EF4\u5EA6\u3002\u4EC5\u6309\u56FA\u5B9A\u8DEF\u7531\u6267\u884C\uFF0C\u4E0D\u8BBE\u7F6E\u63D2\u4EF6\u8C03\u7528\u6B21\u6570\u4E0A\u9650\uFF0C\u4E5F\u4E0D\u63A5\u53D7\u52A8\u6001 MCP \u540D\u79F0\u6216\u8DE8\u4F01\u4E1A\u53C2\u6570\u3002", {
    ...
    try {
      ...
      await workflow.finishRun(task.id, callId, outcome, result.isError ? result.error?.message ?? "\u67E5\u8BE2\u5931\u8D25" : void 0, usable ? summarizeResult(data, dimension) : void 0);
