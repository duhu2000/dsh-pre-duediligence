...
describe("npm 与 DSH 安装契约", () => {
  it("安装 MCP 连接器与 npm 发布的访前插件，不再新装旧 OAuth 插件", () => {
    ...
    expect(installScript).toContain('dsh-pre-duediligence@0.1.8')
    ...
    expect(readme).toContain("dsh plugin --profile web add dsh-pre-duediligence@0.1.8")
  ...
  it("包、Host 元数据与 Skill 使用同一发布版本", () => {
    ...
    expect(host).toContain('version: "0.1.8"')
    expect(skill).toContain("version: 0.1.8")
  })
