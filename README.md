...
0.1.31 修复报告保存/历史读取的 JSON 返回错误，新增用户可见工作阶段与嵌套数据摘要；PDF 内置开源中文字体，可离线导出，无需默认配置环境变量。保留继续尽调、报告版本、补充材料、交叉核验、图片导入和多企业计划。导出为保留Markdown标记的正文版，真实上传尚未接通。详见 [发布说明](docs/RELEASE-0.1.31.md)。
...
```sh
dsh plugin --profile web add dsh-pre-duediligence@0.1.31
```
...
2. `dsh-pre-duediligence@0.1.31`：提供左侧智能体入口、原生会话首页、提示词生成器、Host 执行门和 Skill。
...
dsh plugin --profile web add dsh-pre-duediligence@0.1.31 --allow-build=node-pty,dsh-pre-duediligence
dsh web
~~~
...
应始终看到所选基线的 Connector 和 `dsh-pre-duediligence@0.1.31`；仅在显式启用或原本已安装时看到 Sidebar。重启 DSH 并连接“企查查·企业工商”后，在左侧菜单点击“访前尽调”进入初始原生会话。无 Sidebar 时可继续生成提示词、回填/发送草稿并在会话中阅读报告；五项流程按钮会提示安装可选工作台。安装 Sidebar 时，右侧工作台初始仍关闭；点击业务按钮可主动打开，正式发送任务被接纳后会自动展开一次。仅回填不会展开，收起后轮询不会重新打开。其右侧“+”菜单不会列出该入口，普通会话保持 DSH 原样。
