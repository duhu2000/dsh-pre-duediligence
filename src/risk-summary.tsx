...
export function RiskSummary({ task, insights, running }: { task: HostedTask | null; insights: CardInsights; running: boolean }): JSX.Element {
  ...
  return <section className="qccPwCard qccPwRiskSummary" aria-label="风险研判摘要">
    ...
    {available ? <div className="qccPwRiskTiles">{(["红线", "关注", "信息"] as const).map(level => {
      ...
      const items = published ? reportRows.filter(row => isRiskFindingText(row.text)).map(row => ({id:row.text,title:"",text:row.text,time:"",refs:[] as string[]})) : live.filter(row => row.riskLevel === level).map(row => ({id:row.id,title:row.title,text:row.summary,time:row.updatedAt,refs:row.evidenceIds}))
      ...
      return <article key={level} className="qccPwRiskTile" data-level={level} data-clear={noFinding} data-empty={!items.length}>
        <div className="qccPwRiskTileTop"><b>{level === "红线" ? "红线 · 优先核实" : level}</b><strong>{items.length ? items.length : noFinding ? "本次未发现" : "待研判"}</strong></div>
        ...
        {!items.length ? <p>{noFinding ? "报告在本次覆盖范围内未列出该级别事项；不代表风险绝对不存在。" : "尚无该级别判断，不能解释为零风险。"}</p> : null}
      </article>
    })}</div> : null}
  </section>
}
