...
0.1.27 按 DSH-UX-001 v1.5.4 移除首页副标题及占位，大标题下直接保留原生工作区、模式和输入框。固定菜单名称，收紧局部可逆标题恢复，保护用户自定义标题。详见 [发布说明](docs/RELEASE-0.1.27.md)与[采用记录](docs/HOME-STANDARD-1.5.4.md)。
...
```sh
dsh plugin --profile web add dsh-pre-duediligence@0.1.27
```
...
2. `dsh-pre-duediligence@0.1.27`：提供左侧智能体入口、原生会话首页、提示词生成器、Host 执行门和 Skill。
...
dsh plugin --profile web add dsh-pre-duediligence@0.1.27 --allow-build=node-pty,dsh-pre-duediligence
dsh web
~~~
...
应始终看到所选基线的 Connector 和 `dsh-pre-duediligence@0.1.27`；仅在显式启用或原本已安装时看到 Sidebar。重启 DSH 并连接“企查查·企业工商”后，在左侧菜单点击“访前尽调”进入初始原生会话。无 Sidebar 时可继续生成提示词、回填/发送草稿并在会话中阅读报告；五项流程按钮会提示安装可选工作台。安装 Sidebar 时，右侧工作台仍默认关闭，点击业务按钮后才打开对应视图；其右侧“+”菜单不会列出该入口，普通会话保持 DSH 原样。
