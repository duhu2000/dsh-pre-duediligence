export function processingStatus(input: { running: boolean | undefined; error: string | undefined; waiting: boolean; querying: boolean; secondsSinceResult: number; activity?: { phase: "analysis" | "verification" | "writing"; summary: string; updatedAt: string }; now?: number; lastResultAt?: number }) {
  ...
  if (input.activity && Date.parse(input.activity.updatedAt) >= (input.lastResultAt ?? 0)) {
    const age = Math.max(0, Math.floor(((input.now ?? Date.now()) - Date.parse(input.activity.updatedAt)) / 1000))
    const phase = { analysis: "整理经营事实", verification: "核对风险与证据", writing: "撰写尽调报告" }[input.activity.phase]
    return { busy: input.running === true, title: `最近工作阶段：${phase}`, detail: `${input.activity.summary}（${age} 秒前上报${age >= 120 ? "，阶段信息可能已过时；等待新的工作摘要" : ""}）。${input.running === undefined ? "当前运行状态尚未确认。" : ""}` }
  }
