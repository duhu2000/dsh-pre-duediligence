...
0.1.25 支持历史任务独立五阶段回顾，历史清单隐藏阶段按钮；扫描命中随查询实时展示，经营区域展示实际返回的事实与摘要，风险分级列出全部条目。查询成功不等于企业无风险。旧任务未保存的过程字段明确提示缺失。详见 [发布说明](docs/RELEASE-0.1.25.md)、[作用域契约](docs/TASK-SCOPE-HISTORY-CONTRACT.md)和[兼容矩阵](docs/COMPATIBILITY.md)。
...
```sh
dsh plugin --profile web add dsh-pre-duediligence@0.1.25
```
...
2. `dsh-pre-duediligence@0.1.25`：提供左侧智能体入口、原生会话首页、提示词生成器、Host 执行门和 Skill。
...
dsh plugin --profile web add dsh-pre-duediligence@0.1.25 --allow-build=node-pty,dsh-pre-duediligence
dsh web
~~~
...
应始终看到所选基线的 Connector 和 `dsh-pre-duediligence@0.1.25`；仅在显式启用或原本已安装时看到 Sidebar。重启 DSH 并连接“企查查·企业工商”后，在左侧菜单点击“访前尽调”进入初始原生会话。无 Sidebar 时可继续生成提示词、回填/发送草稿并在会话中阅读报告；五项流程按钮会提示安装可选工作台。安装 Sidebar 时，右侧工作台仍默认关闭，点击业务按钮后才打开对应视图；其右侧“+”菜单不会列出该入口，普通会话保持 DSH 原样。
