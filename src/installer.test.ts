...
describe("installer transaction preflight", () => {
  it("installs the fixed base dependencies without adding optional Sidebar", () => {
    ...
    expect(result.calls.map(args => args[4])).toEqual(["dsh-mcp-connector@0.2.32", "dsh-pre-duediligence@0.1.25"])
  ...
  it("adds the paired Sidebar only when the workbench is explicitly enabled", () => {
    ...
    expect(result.status, result.stderr).toBe(0)
    expect(result.calls.map(args => args[4])).toEqual(["dsh-better-sidebar@0.17.1", "dsh-mcp-connector@0.2.32", "dsh-pre-duediligence@0.1.25"])
  ...
  it("requires the verified optional context version for candidate coexistence", () => {
    ...
    expect(matching.calls.map(args => args[4])).toEqual(["dsh-mcp-connector@0.2.37", "dsh-pre-duediligence@0.1.25"])
