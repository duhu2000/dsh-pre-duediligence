...
var isUserish = (node) => /user|human|system|context|steering/i.test(`${node.role ?? ""} ${node.kind ?? ""}`) || node.interrupted === true;
function adoptTaskFromSnapshot(snapshot, sessionId, minimumBaseline = 0) {
  ...
  for (let idx = nodes.length - 1; idx >= minimumBaseline; idx--) {
    ...
    if (node === void 0 || !/^(user|human)$/i.test(node.role ?? node.kind ?? "")) continue;
...
function captureTaskReport(snapshot, sessionId, task) {
  ...
  if (node === void 0 || !/^(user|human)$/i.test(node.role ?? node.kind ?? "")) return null;
...
function riskSteps(events, insights, finished) {
  ...
  return [
    ...
    { label: "\u660E\u7EC6\u4E0B\u94BB", state: stepOf(drillDone, has(events, DRILL), finished), note: !drillDone && finished ? "\u672A\u89C1\u660E\u7EC6\u67E5\u8BE2\u5B8C\u6210\u8BC1\u636E" : void 0 },
...
function PrevisitWorkbenchTab(props) {
    ...
    const refresh = async () => {
      try {
        ...
        if (record === null) {
          ...
          const startedAfter = new Date(task.createdAt).getTime() - 6e4;
          record = candidates.find((candidate) => new Date(candidate.createdAt).getTime() >= startedAfter) ?? null;
        }
        if (disposed) return;
        setHostedTask(record);
        setHostError(void 0);
        if (record?.entity?.fullName !== void 0 && record.entity.fullName !== props.shared.get(sessionId).company) {
          props.shared.update(sessionId, (state) => ({ ...state, company: record.entity?.fullName ?? state.company }));
        }
        if (record === null || !HOSTED_TERMINAL.has(record.state)) timer = setTimeout(refresh, 1e3);
      } catch (error) {
  ...
  (0, import_react6.useEffect)(() => {
    ...
    const refresh = async () => {
      try {
        ...
        setHostError(void 0);
        if (records.some((record) => !HOSTED_TERMINAL.has(record.state))) timer = setTimeout(refresh, 2e3);
      } catch (error) {
        if (!disposed) setHostError(error instanceof Error ? error.message : String(error));
      }
    };
  ...
  (0, import_react6.useEffect)(() => {
    ...
    const refresh = () => {
      ...
      const captured = task === void 0 ? null : captureTaskReport(snapshot, sessionId, task);
