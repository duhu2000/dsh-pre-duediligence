...
function Report({ connection, sessionId, callId, block }) {
  ...
  return <section data-f24-report style={{width:'100%',minWidth:0,border:'1px solid #9ca3af',borderRadius:8,overflow:'hidden'}}>
    <div style={{padding:8,display:'flex',justifyContent:'space-between',gap:8}}><strong>访前报告与证据 · 合成数据实验</strong><button onClick={() => setOpened(v => !v)}>{opened ? '关闭报告' : '重新打开报告'}</button></div>
    {!meta && <p>等待工具返回已保存报告…</p>}
