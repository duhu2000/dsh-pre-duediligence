...
function PrevisitWorkbenchTab(props) {
  ...
  const hostedEvents = (0, import_react6.useMemo)(() => hostedTask === null ? [] : [
    ...
    ...hostedTask.runs.map((run) => ({ name: run.toolName ?? `previsit_${run.dimension}`, status: run.status, ...run.message === void 0 ? {} : { reason: run.message } }))
  ], [hostedTask]);
