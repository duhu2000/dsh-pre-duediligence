  ...
  const startTask = async () => {
    ...
    try {
      ...
      store.update(sessionId, (s) => ({
        ...
        task: { id, prompt, createdAt: (/* @__PURE__ */ new Date()).toISOString(), nodeBaseline, seenRunning: false, selection }
      }));
...
function syncHostedTaskState(state, record, adopted, locateCurrentStage) {
  ...
  if (previous?.id === nextTask.id && previous.captureId === nextTask.captureId && previous.prompt === nextTask.prompt && previous.createdAt === nextTask.createdAt && previous.nodeBaseline === nextTask.nodeBaseline && previous.seenRunning === nextTask.seenRunning && state.company === nextCompany && state.view === nextView) return state;
...
function PrevisitWorkbenchTab(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime6.jsxs)("section", { className: "qccPwShell", "aria-label": "\u8BBF\u524D\u5C3D\u8C03\u5DE5\u4F5C\u53F0", "data-status": status, children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime6.jsxs)("div", { className: "qccPwBody", children: [
      ...
      shared.view === "output" ? /* @__PURE__ */ (0, import_jsx_runtime6.jsx)(DeliveryPanel, { task, status, toolCount: hostedTask?.used ?? runtime.toolNames.length, ...hostedTask === null ? {} : { toolLimit: hostedTask.limit }, failedToolCount: hostedTask?.runs.filter((run) => run.status === "failed").length ?? runtime.failedToolCount, cardCaptured: cardText !== null, reportHtml }) : null,
