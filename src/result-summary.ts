...
import { collectPortrait, type CompanyPortrait } from "./company-portrait.js"
export type ResultSummary = { summary: string; facts: string[]; factors: { name: string; count: number }[]; portrait?: CompanyPortrait }

export function summarizeResult(value: unknown, dimension?: string): ResultSummary {
  const result: ResultSummary = { summary: "", facts: [], factors: [] }
  const seen = new Set<object>()
  const portrait: CompanyPortrait | undefined = dimension === "registration" || dimension === "profile" ? {} : undefined
