...
const isUserish = (node: CardNode): boolean => /user|human|system/i.test(`${node.role ?? ""} ${node.kind ?? ""}`)

// 从会话里认领任务：找最后一条带“访前任务 ID：PV-xxx”的消息（工作台/设定条发出的正式提交）；
// 没有标记但已出现企查查工具调用时，视为“会话内直接发起的尽调”，从头跟踪。
export function adoptTaskFromSnapshot(snapshot: CardSnapshot): { id: string; prompt: string; nodeBaseline: number } | null {
  const nodes = snapshot.nodes ?? []
  for (let idx = nodes.length - 1; idx >= 0; idx--) {
    const node = nodes[idx]
    if (node === undefined || node.kind === "tool-result" || node.kind === "tool-call") continue
  ...
  if (firstTool !== -1) {
    ...
    for (let idx = firstTool - 1; idx >= 0; idx--) {
      ...
      if (t !== "" && !/<system-reminder>/.test(t)) { prompt = t; break }
    }
